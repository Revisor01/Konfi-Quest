...
- [x] **M4 — Bibelübersetzung vereinheitlicht.** ERLEDIGT (PR #93). Alle vier
      ...
      vom 27.08. ganz entfernt, statt es überall zu ergänzen.
      Am 27.08. nach dem Merge nachgemessen: `konfi/views/ProfileView.tsx:51`
      importiert das geteilte `shared/BibleTranslationModal`, die lokale Kopie
      ist weg, RVR60 kommt im Frontend nirgends mehr vor.
      **Merkposten:** `backend/routes/konfi.js` und `teamer.js` führen RVR60
      weiterhin in `validTranslations`. Folgenlos, solange keine Oberfläche es
      anbietet — wer es per API setzt, bekommt aber eine Übersetzung, die die
      App nicht mehr benennen kann.
