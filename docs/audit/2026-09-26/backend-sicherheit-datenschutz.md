...
- **Schwere:** HOCH
- **Status:** behoben 26.09.2026 — `POST /einladungen` lädt die Stammrolle des Ziels mit; ein Konfi bekommt dieselbe 404-Antwort wie eine unbekannte Kennung (kein Name, keine Einladung, kein Push, keine Mail). Team-Konten bleiben exakt auffindbar (Einladung erscheint ohnehin in der Liste). Tests für fremden Konfi (Benutzername, E-Mail), eigenen Konfi, Identität der Antworten und den erlaubten Fall in `einladungen.test.js`.
...
- **Schwere:** HOCH
- **Status:** behoben 26.09.2026 — `passwordResetLimiter` schlüsselt wie alle IP-Limiter über `ipKeyGenerator(clientIp(req))` (5/15 min je Absender); dazu `passwordResetEmailLimiter` mit 3/h je normalisierter Ziel-E-Mail, gezählt unabhängig davon, ob ein Konto existiert (kein Enumerations-Leak). Tests in `tests/routes/passwortResetGrenzen.test.js`: sechs Adressen → keine 429; sechs von einer Adresse → sechste 429; dieselbe E-Mail von vier Adressen → vierte 429; andere E-Mail bleibt frei; bekannte und unbekannte Adresse zählen gleich. Die Produktionsmessung zu `req.ip` (unten Nr. 4) bleibt offen. Nachtrag (Koordination): beide Reset-Zähler liegen jetzt wie die übrigen Limiter im gemeinsamen Datenbank-Store (`rate_limit_zaehler`, Betrieb BF-09) und gelten damit über beide Replicas hinweg; Test „replica-uebergreifend" in `passwortResetGrenzen.test.js`.
...
- **Schwere:** MITTEL
- **Status:** behoben 26.09.2026 — Compose-Referenz mit Stack-Variablen statt Klartext (Host, Mail-Login, IP), Abrissliste und `refresh-schema.sh` ohne Zugangsziel, Adressen in den Audit-Berichten geschwärzt; die Fallback-Werte im Code entfernt (`utils/smtpKonfiguration.js`: `SMTP_HOST`/`SMTP_USER` ausschließlich aus der Umgebung, klare Meldung statt Fallback, Tests in `tests/utils/smtpKonfiguration.test.js`). Die Git-Historie behält die Adressen (siehe BF-02, Widerruf statt Umschreiben). **Vor dem Deploy:** `SMTP_HOST`, `SMTP_USER`, `SMTP_HOST_IP` als Stack-Variablen in Portainer setzen, sonst startet der Mailversand nicht.
...
1. **Klartextpasswörter (BF-06):** `SELECT COUNT(*) FROM konfi_profiles WHERE password_plain IS NOT NULL;` — bei > 0 sofort `UPDATE konfi_profiles SET password_plain = NULL;` und Backups bewerten. *Nachtrag 26.09.2026:* Migration 165 führt das `UPDATE` beim Deploy automatisch aus — die Zählung läuft trotzdem **vorher**, denn nach dem Deploy steht die Zahl nicht mehr in der Datenbank, und nur sie sagt, ob die bestehenden Sicherungen (nächtliche Dumps) Klartext enthalten und zu bewerten sind.
