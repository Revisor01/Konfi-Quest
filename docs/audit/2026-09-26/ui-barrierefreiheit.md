...
- **Schwere:** MITTEL
- **Status:** teilweise behoben 26.09.2026 — die Grautöne: `--app-text-muted` `#999` → `#707070` (auf Weiß 2,85 → 4,95:1), `--app-text-system` `#8e8e93` → `#6e6e73` (3,26 → 5,07:1), `--app-text-tertiary` `#888` → `#6b6b70` (3,54 → 5,30:1); alle ≥ 4,5:1 auch auf `#f8f9fa`, `#f5f5f5` und dem Ionic-Seitengrund `#f4f5f8`, Rangfolge secondary < tertiary < system < muted erhalten (`dunkelmodus.test.ts`, hell wie dunkel). **Offen:** Bereichsfarben als Textfarbe im Hellen (badges 2,15, chat 2,43, users 3,66 …) — die neue Text-Token-Familie `--app-text-<bereich>` ist hell absichtlich die Bereichsfarbe (Dunkelmodus-Paket, Baustein 2: im Hellen ändert sich an den Farben nichts); alle Textstellen hängen jetzt aber an diesen Tokens, sodass ein hellerer Hell-Wert je Bereich an **einer** Stelle gesetzt werden kann. Ebenso offen: weiße Symbole auf den Eck-Marken. **Nachtrag 26.09.2026 (Dunkelmodus-Paket K2):** Im Dunkeln liegen die Eck-Marken jetzt unter einer Schicht Schwarz mit 40 % Deckkraft (weiße Schrift darauf 5,4–9,0:1); hell unverändert offen (2,15–4,23:1). Nebenbefund derselben gerenderten Messung, hier zugehörig und **offen**: die Kopfbanner der Termindetails (`SectionHeader` mit Statusfarbe) — weiße Überschrift auf Erfolgsgrün `#34c759` 2,22:1, Untertitel 1,89, Kennzahlen-Kacheln 1,73; bei „Nur Info“ auf Grau `#9ca3af` 2,54 — in beiden Modi, iOS wie Android (`docs/audit/2026-09-26/darkmode.md`, Abschnitt Paket K2).
...
- **Schwere:** NIEDRIG
- **Status:** behoben 26.09.2026 — ein globaler Block in `theme/barrierefreiheit.css` unter `prefers-reduced-motion: reduce` setzt `animation-duration`/`transition-duration` auf 0,01 ms und `animation-iteration-count` auf 1 (`!important`) für alle Klassen mit Präfix `app-` samt `::before`/`::after` und für jedes Element mit Inline-`animation`/`transition` (Ladepunkte, Abzeichen-Puls, Karten); Ionics Seitenübergänge über `animated: !bewegungReduziert()` in `setupIonicReact`, der Wisch der Einführung über Swiper `speed 0` (`utils/bewegung.ts`). Wrapped bleibt unberührt (Klassen `wrapped-`/`teamer-`/`konfetti-`, keine Inline-Animation, seine drei eigenen Blöcke bestehen weiter); der „Was ist neu"-Block in `variables.css` bleibt. Gemessen (grep `prefers-reduced-motion` in `src`, ohne Tests): 2 Dateien/4 Stellen → 4 Dateien/6 Stellen; Theme-Regeln mit `animation:`/`transition:` ohne `app-`-Selektor: 0. Tests `bewegungsreduktion.test.ts` (Block, Abdeckung aller Theme-Animationen, Wrapped-Verträglichkeit, App.tsx/OnboardingTour), `bewegungsreduktionGerendert.test.tsx` (Onboarding-Wisch 300 → 0 ms, Ladepunkte mit Inline-Animation), `bewegung.test.ts`.
