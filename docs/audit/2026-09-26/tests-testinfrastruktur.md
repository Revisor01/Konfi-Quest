...
- **Schwere:** MITTEL
- **Status:** behoben 26.09.2026 — `backend/tests/routes/fremdeGemeinde.test.js`: „Konfi aus Org 2 → 404" (Antwort trägt nur `error`, kein Name, kein `qr_token`), „Admin aus Org 2 → 404" und der erlaubte Fall „Konfi aus Org 1 → 200". Gegenprobe wie G12 (Filter in `lesen.js:618` durch `($2::int IS NOT NULL)` ersetzt): der Konfi-Test fällt mit `expected 200 to be 404`, der Admin-Test mit `expected 403 to be 404`; danach Datei zurückgesetzt.
...
- **Schwere:** MITTEL
- **Status:** behoben 26.09.2026 — alle zehn Stellen auf den konkreten Wert: Nr. 1 `rbac.test.js` 404 (Body um `type` ergänzt — ohne ihn endete der Request in der Pflichtfeldprüfung mit 400, bevor der Gemeinde-Filter erreicht war); Nr. 2 `material.test.js` 400 „Keine Dateien hochgeladen"; Nr. 3 `autoDeletion.test.js` `sent` = 0 (kein Leitungskonto im Seed hat eine E-Mail-Adresse, der Marker wird im Zweig „keine Adressaten" gesetzt); Nr. 4 `organizations.test.js` in zwei Tests geteilt — bestehende Sitzung (`generateTokenMitAlter`) → 401 „Token invalidated", frisches Token ohne Mitgliedschaft (Soft-Revoke dafür bewusst zurückgesetzt) → 403 „Kein Zugriff auf diese Organisation"; Gegenprobe G08b wiederholt: beide Soft-Revoke-Vergleiche in `rbac.js` ausgebaut → der 401-Test fällt mit `expected 403 to be 401`, der 403-Test bleibt grün; Nr. 5 `chat.test.js` 415 „Dateityp konnte nicht verifiziert werden" (Titel von „→ 200" auf „→ 415" berichtigt); Nr. 6 `badges.test.js` `level_info.current_level.title` = Novize bzw. Gehilfe (das geprüfte Feld `level` gab es in der Antwort nie); Nr. 7 `tageUndKalendertag.test.ts` mit fester Uhrzeit 10:00 (`vi.setSystemTime`), Erwartung ohne Bedingung; Nr. 8 `App.test.tsx` prüft, dass `ion-app` im DOM steht; Nr. 9 `teamer.test.js` Dashboard-Form per `toEqual` (greeting-Schlüssel, Name, leere Listen, Badge-Zähler 0/0, vollständige config samt section_order); Nr. 10 `auth.test.js` 400 und 404 `not_found`, `badges.test.js:333` 400, `challenges.test.js` 404 „Challenge nicht gefunden". Kein heutiger Wert war falsch, darum kein Eingriff in eine Route. Die übrigen `>= 0`-Stellen aus der Zählung (Indexsuche in `wrappedKacheln.test.js:631`, `verbindungFreigabe*.test.js`; Zähler in `appIconBadge.test.js:188`, `wrapped.test.js:2274`, `badges.test.js:657`) gehören nicht zu den zehn und sind unverändert.
- **Fundstelle:** siehe Liste
...
- **Schwere:** MITTEL
- **Status:** behoben 26.09.2026 — Die 19 Fehler sind behoben (ungenutzte Importe und Variablen entfernt, `catch`-Bindungen weggelassen, drei Test-Hilfskomponenten reichen den Kontext im Effect statt beim Rendern nach außen, ein `require` ist ein Import); der Lint-Schritt in `ci.yml` läuft bei Push und Pull Request über den ganzen Frontend-Baum, nur Fehler blockieren, der veraltete Zahlen-Kommentar ist ersetzt.
