...
| `category_combination` | Aus wie vielen VERSCHIEDENEN Kategorien war jemand dabei (jede zählt höchstens einmal) | `criteria_value`, `criteria_extra.required_categories` (Namen, mindestens zwei) | konfi + teamer | Ja seit 06.09.2026, beide Rollen. Zähl-Queries und Zählfunktion liegen gemeinsam in `utils/badgeKategorieRegel.js`, Wertung und Fortschritt nutzen dieselbe Funktion. Rollentrennung wie bei `category_activities`: Konfi zählt Konfi-Aktivitäten + freiwillige besuchte Termine, Teamer nur `target_role='teamer'`-Aktivitäten + alle anwesenden Termine |
