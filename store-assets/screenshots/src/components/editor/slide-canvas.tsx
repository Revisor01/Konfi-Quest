...
function backgroundFor(theme: Theme, inverted?: boolean) {
  ...
  // Login-BG-Stil, eingefärbt in der jeweiligen Sektionsfarbe:
  // weisser Glow oben links + heller Akzent-Glow unten rechts + diagonale
  // Aufhellung von Grundfarbe -> heller -> Akzent. Hell & leuchtend.
  return [
    `radial-gradient(circle at 15% 8%, rgba(255,255,255,0.30) 0%, transparent 45%)`,
    `radial-gradient(circle at 92% 100%, ${theme.accent} 0%, transparent 52%)`,
    `linear-gradient(150deg, ${shade(theme.bg, 6)} 0%, ${shade(theme.bg, 14)} 38%, ${shade(theme.bg, 22)} 65%, ${theme.accent} 130%)`,
  ].join(", ");
}
...
function getDefaultRects(
  ...
  // Vom User kalibriert (iOS Slides 1-5): gerade Geräte ~83% breit, Oberkante
  // bei ~30%; device-top ~92% breit, oben angeschnitten bei ~-17%.
  const tallW = deviceW;
  const tallH = deviceH;
  const tallTopY = cH * 0.30;          // gerade: Oberkante 30%, unten auslaufend
  const tallTopOverY = -cH * 0.175;    // device-top: oben angeschnitten

  switch (layout) {
    case "hero":
      return {
        caption: { x: cW * 0.08, y: cH * 0.07, width: capW, height: capH, align: "center" },
        device: {
          // Schlank & hoch: Oberkante ~33%, unten auslaufend (~3/4 sichtbar).
          x: (cW - tallW) / 2,
          y: tallTopY,
          width: tallW,
          height: tallH,
    ...
    case "device-top":
      return {
        caption: { x: cW * 0.08, y: cH * 0.74, width: capW, height: capH, align: "center" },
        device: {
          // Oben angeschnitten, unteres ~3/4 sichtbar, Headline darunter.
          x: (cW - tallW) / 2,
          y: tallTopOverY,
          width: tallW,
          height: tallH,
...
export function SlideCanvas({
  ...
  // Per-slide palette override ("bold & colourful per feature"): derive an
  // effective theme so a single slide can carry its own bg/fg/accent without
  // changing the project theme. bgAlt/fgAlt stay from the base theme so the
  // inverted variant still works.
  const theme: Theme = slide.palette
    ? {
        ...baseTheme,
        bg: slide.palette.bg ?? baseTheme.bg,
        // bgAlt folgt der Palette-bg, damit auch Layouts die bgAlt nutzen
        // (Feature-Graphic) die fetzige Feature-Farbe bekommen.
        bgAlt: slide.palette.bg ?? baseTheme.bgAlt,
        fg: slide.palette.fg ?? baseTheme.fg,
        accent: slide.palette.accent ?? baseTheme.accent,
        muted: slide.palette.muted ?? baseTheme.muted,
      }
    : baseTheme;
  ...
  // Getiltete Slides (vom User kalibriert): grösser ~98% Breite, höher sitzend
  // (~25%). Links- und Rechts-Schräg sind symmetrisch — gleiche Grösse, nur
  // Tilt-Vorzeichen + gespiegelter horizontaler Versatz in Kipprichtung.
  if (slide.tilt && defaults.device && (slide.layout === "device-bottom" || slide.layout === "hero")) {
    const d = defaults.device;
    const newW = cW * 0.98;
    const newH = newW / frameAspect;
    const dir = slide.tilt > 0 ? 1 : -1;      // +rechts / -links
    const shift = cW * 0.06 * dir;            // in Kipprichtung verschieben
    defaults.device = {
      ...d,
      x: (cW - newW) / 2 + shift,
      y: cH * 0.25,
  ...
  function renderDevice(id: "device" | "deviceSecondary", rect: Rect, src: string, extraStyle?: React.CSSProperties) {
    const saved = slide.transforms?.[id];
    // slide.tilt gibt dem primären Gerät einen Default-Winkel (schräg/dynamisch),
    // sofern keine explizite Rotation im Transform gesetzt ist.
    const tiltDefault = id === "device" ? (slide.tilt ?? 0) : 0;
    const rotation = saved?.rotation ?? tiltDefault;
  ...
  return (
    ...
    >
      ...
      {/* Freigestelltes Logo (Lutherrose+Herz) als sehr grosses, angedeutetes
          Wasserzeichen im Hintergrund (opt-in via showIcon). Bricht oben rechts
          aus dem Rahmen, einfarbig weiss, niedrige Deckkraft. */}
      {slide.showIcon && img("/logo-mark.png") && (
        <img
