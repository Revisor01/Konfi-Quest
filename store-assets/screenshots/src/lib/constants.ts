...
// Vom User kalibriert (iOS Slides 1-5): gerade Geräte ~83% Canvas-Breite,
// schräge ~98%. phoneW liefert die gerade-Breite; der Tilt-Block skaliert hoch.
export function phoneW(cW: number, cH: number, clamp = 0.83) {
  return Math.min(clamp, 0.83 * (cH / cW) * MK_RATIO);
}
