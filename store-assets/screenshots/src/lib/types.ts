...
export type Slide = {
  ...
  // Per-slide colour override ("bold & colourful per feature"). When present,
  // these win over the project theme for THIS slide only. Any subset is allowed.
  palette?: { bg?: string; fg?: string; accent?: string; muted?: string };
  // Default device tilt in degrees (e.g. -6 / 8) for a dynamic, angled look.
  // Applied to the device frame unless a per-element rotation is set in transforms.
  tilt?: number;
  // Show the app icon as a large, faint watermark in the background of this slide.
  showIcon?: boolean;
