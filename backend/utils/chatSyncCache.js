// In-Memory-TTL-Cache für den Chat-Mitgliedschafts-Sync (Audit Achse 4, Fund 1).
//
...
// Stand passen — das ändert sich selten und muss nicht pro Request geprüft
// werden.
//
// Warum der TTL sicher ist: Ein NEUER User hat keinen Cache-Eintrag — sein
// erster /rooms-Aufruf synct ihn sofort in seine Räume. Bestehende Mitglieder
...
// greift der Sync spätestens nach Ablauf des TTL. Bewusst akzeptiert.
// invalidate() ist für kuenftige Mutations-Pfade ohne Inline-Sync exportiert.
...
// Muss der Sync für diesen User (wieder) laufen?
...
// Bei Jahrgang-/Rollenwechsel eines Users aufrufen: Der nächste /rooms-Aufruf
// synct dann sofort neu (statt bis zu TTL zu warten).
function invalidate(orgId, userId) {
  lastSync.delete(key(orgId, userId));
}

// Nur für Tests: kompletten Cache leeren (DB wird zwischen Tests getruncated,
// der Modul-Cache wuerde sonst Sync-Läufe faelschlich ueberspringen).
function clear() {
  lastSync.clear();
}
