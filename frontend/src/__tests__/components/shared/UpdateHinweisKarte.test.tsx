...
describe('UpdateHinweisKarte', () => {
  ...
  it('zeigt Titel und Untertitel im "Was ist neu"-Look', () => {
    setup();
    expect(screen.getByText('Was ist neu in Version 2.2?')).toBeInTheDocument();
    expect(
      screen.getByText('App-Sperre per Face ID, Challenge-Stempel im Profil, schnellerer Start — hier tippen für den Überblick.')
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: 'Was ist neu in Version 2.2? Die Neuerungen ansehen' })
